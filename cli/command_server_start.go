...
func (c *commandServerStart) run(ctx context.Context) (reterr error) {
	...
	if err := c.setupHandlers(srv, m); err != nil {
		return err
	}
...
func (c *commandServerStart) setupHandlers(srv *server.Server, m *mux.Router) error {
	for _, h := range serverExtraHandlers { // warphold: fleet routes, registered before the UI catch-all
		if err := h(srv, m, c.svc.repositoryConfigFileName()); err != nil {
			return err
		}
	}
