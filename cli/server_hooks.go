...
var serverExtraHandlers []func(srv *server.Server, m *mux.Router, configFile string) error

// RegisterServerHandlers adds a function that mounts routes when `server start` builds its router.
func RegisterServerHandlers(f func(srv *server.Server, m *mux.Router, configFile string) error) {
