...
5. Set Fleet's own user as maintenance owner (`maintenance set --owner=<fleet identity>`, existing `initialize` behaviour) **and** run the device's engine with `--no-auto-maintenance`, so **devices run no maintenance** — D3. Spike (Task 1) proved this is what removes the only overwrite Kopia otherwise issues (`kopia.maintenance`).
...
0000. **RULING (Task 8 review) — seventh upstream touch:** `internal/server/server.go` gains `Options.DisableMaintenance` (two `// warphold:` hunks). Upstream's `--no-auto-maintenance` only reaches the CLI write-session path; the server's maintenance manager starts unconditionally and would fail-and-notify on every tick on a device repo whose maintenance owner is the Fleet identity. No non-upstream hook exists (`s.maint` is unexported), so the option is the minimal, upstreamable change. The touched-upstream list is now seven files.

000. **RULING (Task 16 review):** deb/rpm packages never provision a Fleet server — no system user, no `/var/lib/warphold`, no unit; `postinstall` only prints the two next steps. Server provisioning is `fleet.sh`'s job alone, so `apt install warphold` on a desktop is inert and safe. Packages install to `/usr/bin`, tarball installers to `/usr/local/bin`; never install both. Packaging-family detection in `fleet.sh` is dropped (tarball on every distro).
...
1. **RESOLVED by Task 1 (`docs/RECONCILE-append-only.md`):** devices delete exactly one class, session markers `^s[0-9a-f]{16,}` (hard error if denied, no flag); overwrite none once the maintenance owner is the Fleet identity and the device runs `--no-auto-maintenance`. Allowlist: `allowDelete = ["^s[0-9a-f]{16,}"]`, `allowOverwrite = []`. The D4 spike's output is the source of truth for §4.2's delete allowlist. This spec deliberately does not name the classes: `repo/blob/s3/s3_storage.go:223` shows `DeleteBlob` → `RemoveObject`, and Kopia's session (`s`) blobs and index compaction are the suspected callers, but the allowlist is written from the spike's recorded log, not from that guess. Until then, every task that mentions the allowlist refers to "the classes recorded by Task 1".
2. **RESOLVED by Task 1:** `UNSIGNED-PAYLOAD` on PUT over TLS, real digest elsewhere, `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` only over plain HTTP (gateway answers 501; it is always behind TLS); `Content-MD5` on every PUT; no multipart (largest single PUT observed 21.4 MB); single-range GET. minio-go's payload signing mode. `x-amz-content-sha256` may be a real digest, `UNSIGNED-PAYLOAD`, or `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` depending on TLS and object size. The verifier must handle whichever the client actually sends; the spike's request log settles it. Streaming chunked uploads need chunk-signature verification, which is meaningful extra work — if the client uses them, that is a scoping decision to raise, not to absorb silently.
