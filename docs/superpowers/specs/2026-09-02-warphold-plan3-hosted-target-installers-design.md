...
| D8 | **Distribution:** goreleaser on tags publishes tarballs + deb + rpm + signed checksums to GitHub Releases. **`get.warphold.com`** (GitHub Pages behind Cloudflare; Hody owns `warphold.com` as the canonical domain and `warphold.dev` as a redirect) serves `fleet.sh` and `app.sh`, pinned to the latest release. Fleet servers keep serving `enroll.sh` and `/dl/` from their own domain, built from `public_url`. **M6 moves the two scripts under `warphold.com/get/` and turns `get.warphold.com` into a Cloudflare redirect rule** to the same paths; `curl -fsSL` follows it (`-L`), so both one-liners keep working verbatim (§10.5.3). | §8, §10.5. |
...
| CSRF | the origin check compares `Origin`/`Referer` against `public_url`'s scheme+host |
...
`.goreleaser.yml` on a tag produces, for `linux/{amd64,arm64}`: a tarball, a `.deb`, a `.rpm`, and `checksums.txt` signed by `tools/sign.sh` (the signing key is a HUMAN CHECKPOINT — it lives in 1Password and is provided to CI as a secret). `nfpms` already exists; it gains WarpHold's homepage/vendor/maintainer/description, a `postinstall` that creates the `warphold` user for the system-scope case, and `conflicts: kopia`.

`get.warphold.com` is a GitHub Pages site in a small public repo (`hodyhq/get.warphold.com`), fronted by Cloudflare, serving exactly two scripts plus an `index.html` that shows both one-liners. Each script resolves the latest release through the GitHub API once and pins the version it downloaded into its own output, so a re-run is reproducible and a failure is loud.
...
`scripts/screenshots.sh` in the UI repo, driving a small headless-browser script (Playwright if it is already in `devDependencies`; otherwise the chrome-devtools driver already used for verification — **no new heavy dependency** is added for this). It:
...
- `/get/fleet.sh` and `/get/app.sh` are served from the site.
- **DNS (HUMAN CHECKPOINT):** Cloudflare — apex and `www` → Pages; `get.warphold.com` → a redirect rule to `https://warphold.com/get/…`; `warphold.dev` → 301 to `warphold.com`. GitHub Pages custom-domain verification and HTTPS enforcement are part of the same checkpoint, as is a `CNAME` file in the repo.
...
- **UI:** vitest for the new screens, Playwright for wizard-to-enrolled and kit-ack-clears-the-banner.
...
| M6 | Screenshot pipeline, both READMEs, `warphold.com` site, DNS and redirects | `https://warphold.com` serves both tabs with real screenshots and `curl -fsSL https://get.warphold.com/fleet.sh` still works — **its own PRs**, parallel with M4/M5 |
...
4. **Kopia's S3 backend flags on current master** — `--endpoint`, `--access-key`, `--secret-access-key`, `--region`, `--disable-tls` — must be checked against `cli/command_repository_create_s3.go` (or its current equivalent) before they are printed in a recovery kit, because the kit is the one artefact that must work years later with an unfamiliar binary.
...
6. **GitHub Pages + Cloudflare** for `get.warphold.com` (M3) and then `warphold.com` (M6): DNS, the Pages custom-domain verification flow, HTTPS enforcement, whether Cloudflare's proxy needs a cache rule to keep the install scripts from going stale, and whether the `get.` redirect rule preserves the path as written. HUMAN CHECKPOINT in the plan.
9. **What headless-browser tooling the UI repo already has.** The screenshot pipeline uses whatever is in `devDependencies` (Playwright, or the chrome-devtools driver already used for verification). Check before writing the script; do not add a heavy dependency for screenshots.
