...
func buildDigest(ctx context.Context, st *store.Store, now time.Time) (*digestData, error) {
	...
	mirrorEvery := mirrorIntervalSetting(ctx, st)
...
// mirrorIntervalSetting reads the mirror job's own cadence, in the seconds
// convention the scheduler stores it in, so the offsite staleness rule tracks
// whatever this fleet actually mirrors on rather than a number of its own.
func mirrorIntervalSetting(ctx context.Context, st *store.Store) time.Duration {
	v, err := st.Setting(ctx, "mirror_interval")
	if err != nil || v == "" {
		return intervals["mirror"].def
	}
