...
func TestStatsRecordsRepositorySize(t *testing.T) {
	...
	detail, err := Stats(fx.st, fx.key)(ctx, store.Job{Kind: "stats"})
...
func TestStatsScopesToOneAgent(t *testing.T) {
	...
	detail, err := Stats(fx.st, fx.key)(ctx, store.Job{Kind: "stats", AgentID: "ag_1"})
	require.NoError(t, err)
...
func TestStatsPreservesMirrorProgress(t *testing.T) {
	...
	_, err := Stats(fx.st, fx.key)(ctx, store.Job{Kind: "stats"})
	require.NoError(t, err)
