...
// isPathSegment reports whether s is one plain path element.
func isPathSegment(s string) bool {
	return s != "" && s != "." && s != ".." &&
		!strings.ContainsAny(s, `/\`) && filepath.Clean(s) == s
}
...
func retentionWindow(ctx context.Context, st *store.Store) time.Duration {
	...
	if v, err := st.Setting(ctx, revokedRetentionSetting); err == nil && v != "" {
