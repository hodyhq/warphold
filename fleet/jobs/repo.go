...
// CloudStoreFn opens the write-through backend of a cloud-direct hosted
// target: the same handle the gateway serves that target's devices from. It is
// supplied by fleet/api (cloudStoreFor), which owns the sealed credentials --
// fleet/api imports this package, so the dependency can only run this way
// round. It is the same injection fleet/enroll uses for provisioning.
type CloudStoreFn func(ctx context.Context, t *store.Target) (gateway.ObjectStore, error)

// fleetConnection returns the connection the FLEET uses to open one device's
// repository directly, or ok=false when the escrowed connect token already
// names a direct one (a "filesystem" or "b2" target: the token's connection
// info IS the storage).
//
// A hosted target is the exception, and the whole reason this exists. Its
// escrowed token names the DEVICE's view: an "s3" connection to the Fleet's
// own public URL, signed with the device's gateway credential. Opening that
// server-side would mean the Fleet making an HTTP round trip through its own
// gateway -- needing public_url to resolve, TLS to be trusted and the listener
// to be up -- and the maintenance job, which rewrites blobs, would be refused
// by the append-only rule that credential carries. So the Fleet opens the same
// bytes directly instead: the "warphold-hosted" blob adapter for a disk
// target, and the cloud-direct backend for a cloud one. Both are byte-for-byte
// the repository the gateway serves the device (fleet/gateway/blobadapter.go).
func fleetConnection(ctx context.Context, st *store.Store, cloud CloudStoreFn, a store.Agent, prefix string) (blob.ConnectionInfo, bool, error) {
	g, err := st.Group(ctx, a.GroupID)
	if err != nil {
		return blob.ConnectionInfo{}, false, fmt.Errorf("group %d: %w", a.GroupID, err)
	}

	t, err := st.Target(ctx, g.TargetID)
	if err != nil {
		return blob.ConnectionInfo{}, false, fmt.Errorf("target %d: %w", g.TargetID, err)
	}

	if t.Kind != "hosted" {
		return blob.ConnectionInfo{}, false, nil
	}

	if prefix == "" {
		// The bundle always carries it; this is the belt to its braces, and it
		// is the same prefix fleet/enroll minted.
		prefix = a.ID + "/"
	}

	if t.StorageMode == "cloud" {
		if cloud == nil {
			return blob.ConnectionInfo{}, false, errors.New("this fleet cannot open cloud-direct repositories server-side")
		}

		objs, err := cloud(ctx, t)
		if err != nil {
			return blob.ConnectionInfo{}, false, fmt.Errorf("opening the backend of target %q: %w", t.Name, err)
		}

		ci, ok := gateway.RepositoryConnection(objs, prefix)
		if !ok {
			return blob.ConnectionInfo{}, false, errors.New("the cloud-direct backend cannot name a repository connection")
		}
	...
	if t.Path == "" {
		return blob.ConnectionInfo{}, false, fmt.Errorf("hosted target %q has no path", t.Name)
	}

	return blob.ConnectionInfo{
		Type:   gateway.HostedStorageType,
		Config: &gateway.HostedOptions{Root: t.Path, Prefix: prefix},
	}, true, nil
}
...
// only credential path - jobs never ask an agent for anything. For a hosted
// target the connection itself comes from fleetConnection instead, because the
// token's is the device's HTTP view of the gateway; the password is still the
// token's.
//
...
// removes on close.
//
// Mutual exclusion note: Kopia's maintenance lock (.mlock) is keyed to the
// config file, and every job here connects through a fresh scratch config, so
// two Fleet processes sharing one state directory would NOT be kept apart by
// it -- they would maintain the same repository concurrently. The real
...
func openAgentRepo(ctx context.Context, st *store.Store, cloud CloudStoreFn, k seal.Key, a store.Agent, id identity, readOnly bool) (*openedRepo, error) {
	...
	if direct, ok, err := fleetConnection(ctx, st, cloud, a, b.Prefix); err != nil {
		return nil, err
	} else if ok {
		ci = direct
	}
