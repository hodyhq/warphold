...
func TestVerifyPassesOnAGoodRepository(t *testing.T) {
	...
	detail, err := Verify(fx.st, fx.key)(context.Background(), store.Job{Kind: "verify"})
...
func TestVerifyRunsOnlyTheAgentTheJobNames(t *testing.T) {
	...
	detail, err := Verify(fx.st, fx.key)(context.Background(), store.Job{Kind: "verify", AgentID: "ag_1"})
	require.NoError(t, err)
...
func TestVerifySkipsARevokedAgent(t *testing.T) {
	...
	detail, err := Verify(fx.st, fx.key)(ctx, store.Job{Kind: "verify"})
	require.NoError(t, err)
	require.Equal(t, "verified 1/1 ok; 0 failed", detail)
}
...
func TestVerifyStopsWhenTheContextIsDone(t *testing.T) {
	...
	_, err := Verify(fx.st, fx.key)(ctx, store.Job{Kind: "verify"})
